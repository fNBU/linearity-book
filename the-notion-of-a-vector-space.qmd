...
Note that the notation $+ \colon V \times V \to V$ implies that $V$ is, in particular, closed under addition (since $V$ is the conomain of $+$; addition cannot by definition return an element not in $V$).
