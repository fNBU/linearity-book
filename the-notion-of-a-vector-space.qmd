...
It turns out that there is no way (all the proof is hard) to turn that set into a vector space.
...
- a set $A$ that fails to satisfy any of the properties above **can never be used** as the domain or codomain of a linear function. If you tried to use it in this way, you would eventually run into statements that make no sense.
...
$$
\mathbb Q := \left\{ \left. \frac{a}{b}  \right| a,b \in \mathbb R ,  b \ne 0  \right\}.
$$
