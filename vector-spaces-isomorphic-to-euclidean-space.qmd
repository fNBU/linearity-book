...
> 
> Using your choses bases, what vector is $(1,2,3) \in \mathbb R^3$ when interpreted as coordinates for a vector in $P_2$?
...
The inverse of this function is also linear.

:::

The fact that the coordinate function $\Phi : V \to F^n$ and its inverse are linear imply that addition and scalar multiplication in $V$ and $F^n$ are compatible.
That is, we can add vectors in $V$, or we can convert to coordinates, add in $F^n$ and convert back to $V$ and get the same answer.
...
Note that the $+$ symbol on the top of the diagram is the vector addition operation in $V$.
The $+$ symbol on the bottom is vector addition in $F^n$.

The same rule applies for scalar multiplication.
...
\begin{tikzpicture}
  ...
  \path[-stealth]
    (m-1-1) edge node [left] {$\Phi$} (m-2-1)
            edge node [above] {$\cdot$} (m-1-2)
    (m-2-1) edge node [above] {$\cdot$} (m-2-2)
    (m-1-2) edge node [right] {$\Phi$} (m-2-2)
    ;
\end{tikzpicture}
:::

Note that the $\cdot$ symbol on the top of the diagram is the scalar multiplication operation in $V$.
The $\cdot$ symbol on the bottom is scalar multiplication in $F^n$.

::: {#exr-basis-dependence}
...
What @thm-isomorphism-up-to-dim says is that, once we choose a basis for $P_1$, we can treat it *the same as* $\mathbb R^2$.
...
::: {#exr-addition-example}
> Consider 
> $$
> v=3x^3 + 5x^2-7x+1 , w=-4x^3 +2x^2 - 4x + 7 
> $$
> in the vector space $P_3$.
> 
> 1. Compute the polynomial $v+w$.
> 1. Choose a basis for $P_3$. Write $v$ and $w$ as lists of coordinates in this basis.
> 1. Add these two lists of coordinates using addition as it is defined in $\mathbb R^4$. 
> 1. Write $v+w$ (your answer in part 1) as a list of coordinates in your basis for $P_3$.
> 1. Did you get the same answer in the previous two parts? If the answer is no, go back and check your work.
:::
...
> v= 2x^2-2x+7 
> $$
> in the vector space $P_2$.
> 
> 1. Compute the polynomial $5v$.
> 1. Choose a basis for $P_2$. Write $v$ as a list of coordinates in this basis.
> 1. Multiply this list of coordinates by $5$ using scalar multiplication as it is defined in $\mathbb R^3$. 
> 1. Write $5v$ (your answer in part 1) as a list of coordinates in your basis for $P_2$.
> 1. Did you get the same answer in the previous two parts? If the answer is no, go back and check your work.
:::